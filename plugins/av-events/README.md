...
| `memory.recalled` | `recall:memory.recalled` on the plugin event bus (published by `plugins/recall`) | `query_hash`, `hit_count`, `top_score`, `surface` — see below |
...
(`hermes_cli/plugins.py` `emit`/`subscribe`; `hermes_cli/plugins_dispatch.py` `_dispatch_event`,
`_deliver_event`; present in the Hermes tree at the last commit before 2026-09-01, `82e6c46`, and at `0.21.3` / 2026.9.14). `register()` subscribes only when
`ctx.subscribe` exists.

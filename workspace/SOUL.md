...
- **Catch-up uses memory first.** For broad memory, forum, or chat catch-up prompts — "what's going on in the forum", "what did I miss", "catch me up", "what's been happening" — first use the memory read layer: `enzyme catalyze -p agent-memory-vault -n 8 "<user prompt>"` before broad file lookup or live-source fetches. Then verify cited files or live sources before answering. If retrieval fails or has no citations, fallback is allowed, but don't claim retrieval was used. Forum/IRL notes are agent-written observations, not user-authored truth.

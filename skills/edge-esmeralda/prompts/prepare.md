...
1. **Run the deterministic staging script.** Do not compose the brief yourself, do not write ad-hoc Python/JavaScript, do not shell-quote a body, do not call `hermes kanban create` or `hermes kanban block` directly, and do not invoke any Index MCP tool manually. The script fetches opportunities from the Index HTTP API directly using the resident's `INDEX_API_KEY` — no MCP tool call by you is needed or wanted. Run exactly:
   ...
   The script resolves the America/Los_Angeles date, fetches opportunities from the Index API, builds structured context, composes the markdown body, runs the URL guard, creates the Kanban task with argv-safe `--body`, blocks it for review, and records `prepared.taskId` in `memory/heartbeat-state.json`. Its JSON stdout is for diagnostics only; do not expose it.

...
- Index people sections are fetched directly by the script from the Index MCP server using `INDEX_API_KEY` — you never need to call `list_opportunities` or write `memory/digest-opportunities.txt` yourself.

...
Index background work is installed as Hermes/OpenClaw cron jobs: **memory signal sync at `0 1 * * *`**, **digest prepare at `0 2 * * *`**, **digest send at `0 8 * * *`**, **opportunity drops at `0 12 * * *` and `0 17 * * *`**, **negotiation summary at `0 14 * * *`**, and **evening questions at `0 19 * * *`** (host-local). Memory signal sync is script-gated: a deterministic local preflight hashes `MEMORY.md` and suppresses the LLM turn when memory has not changed since the last successful sync. To install the jobs at different times (a different timezone, a test window, etc.), pass full 5-field cron expressions. A flag wins over the matching env var; an invalid expression is ignored with a warning and the default is kept.
...
| Opportunity drop (midday) | `--opportunity-drop-midday-cron "<expr>"` | `OPPORTUNITY_DROP_MIDDAY_CRON` | `0 12 * * *` |
| Opportunity drop (evening) | `--opportunity-drop-evening-cron "<expr>"` | `OPPORTUNITY_DROP_EVENING_CRON` | `0 17 * * *` |
...
Index background work runs as fixed cron prompts — **memory signal sync `0 1 * * *`, prepare `0 2 * * *`, send `0 8 * * *`, opportunity drops `0 12 * * *` and `0 17 * * *`, negotiation summary `0 14 * * *`, and evening questions `0 19 * * *`** (host-local) — and the end user can't change those schedules from chat. Memory signal sync has a deterministic `MEMORY.md` hash preflight, so unchanged days do not wake the model. The Agent Plaza selfie sender is run explicitly by operators for one-off closeout sends and has a deterministic opt-in/packet/media/Telegram preflight, so unavailable Plaza or missing Telegram config stays silent. The installer can override cron times (see "Overriding the Index cron times" under **Install**). The 30-minute heartbeat cron and former Agent Plaza selfie cron were retired.

| You want to… | Edit | Notes |
|---|---|---|
| Override the Index cron times for one install | the matching flag/env from the install table above | Optional, full 5-field cron expressions. Flag wins over env; invalid values fall back to the default. |
...
| Change a cron prompt without changing the schedule | the matching prompt file (`skills/index-network/heartbeat.md` or `skills/edge-esmeralda/prompts/<name>.md`) | Hermes stores prompt copies in cron jobs. Hosted residents are refreshed by an explicit per-tenant update from the control plane (see `docs/deployment.md`; the former `post-merge` workflow was retired on 2026-09-25: its `/hooks/sync` target no longer exists in the control plane, and a merge to `main` changes no running resident), which reruns the installer. For non-control-plane installs or recovery, run `HERMES_HOME=<resident-home> bun install/reconcile_digest_crons.ts` after updated skill files are copied. |
...
| Change the Agent Plaza selfie preflight/follow-up context | `skills/agent-plaza/scripts/agent_plaza_selfie.py` + `skills/agent-plaza/prompts/selfie.md` + `skills/agent-plaza/SKILL.md` | The one-off script reads configured Plaza packets or a local ops handoff, writes only under `ops/agentvillage/...`, stores sanitized `lastFollowupContext` on successful sends, and should emit `wakeAgent:false` when Plaza is unavailable. |
